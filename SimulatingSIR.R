...
mod_sir <- function(ini,
  ...
  for(t in 2:nrow(cases)){
      p_SI <- (1-exp(-x$beta*sim[2]))*(x$beta*sim[2])/(x$beta*sim[2] + 
                                                         x$betap*sim[3] + x$nu)
      p_SIn <- (1-exp(-x$betap*sim[3]))*(x$betap*sim[3])/(x$beta*sim[2] + 
                                                            x$betap*sim[3] + x$nu)
      p_SV <-  (1-exp(-x$nu))*(x$nu)/(x$beta*sim[2] + x$betap*sim[3] + x$nu)
      
      #probability of transitions from first infected compartment
      p_IR <- (1- exp(-x$gamma))*(x$gamma/(x$gamma+x$mu))
      p_ID <- (1-exp(-x$mu))*(x$nu/(x$gamma+x$mu))
      ...
      p_InRn <- (1- exp(-x$gammap))*(x$gammap/(x$gammap+x$mu))
      p_InD <- (1-exp(-x$mu))*(x$nu/(x$gammap+x$mu))
...
ini <- c(s=1-vaxrate-initial_infected,i1=0.99*initial_infected,i2=0.01*initial_infected,v=vaxrate,r1=0,r2=0,d=0)
x <- list(beta=6, betap=6, gamma=1.4, gammap=1.4,mu=0.0002, nu=0.0005)
...
test_run |> mutate(infected = i1+i2,infprop1=i1/infected,infprop2=i2/infected) |>
  ggplot(aes(x=time,y=infprop1)) + geom_point(size=2) + geom_point(aes(x=time,y=infprop2),color="blue", size=2) + 
